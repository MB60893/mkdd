...
"""
Originally created by Mrkol: https://github.com/Mrkol
Configured by MJ (https://github.com/MB60893) for the Mario Kart Double Dash decomp.

Adapted from script used in Super Mario Sunshine decompilation:
https://github.com/doldecomp/sms/blob/main/tools/validate-symbol-order.py


Validate that a translation unit's function symbol order in our compiled object
file matches the order recorded in the original linker map (debugInfoM.MAP or debugInfoS.MAP).
...
def main() -> None:
    ...
    ap.add_argument("-u", "--unit", required=True,
                    help="objdiff unit name, e.g. main/Shiraiwa/MapObjDossun")
